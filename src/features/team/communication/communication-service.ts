...
export async function getAnnouncements(organizationId: string) {
  ...
  const { data, error } = await supabase.from("announcements").select("id,title,body,category,created_by,published_at,expires_at").eq("organization_id", organizationId).order("published_at", { ascending: false });
