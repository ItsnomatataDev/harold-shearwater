  ...
  return <div className="animate-in">
    ...
    <div className="mt-7 grid gap-5 xl:grid-cols-[minmax(0,1.55fr)_minmax(330px,.75fr)]">
      <div className="space-y-5">
        <section className="card overflow-hidden p-5 sm:p-6"><SectionHeader title="Today's schedule" action="View schedule"/>{data.schedule.length ? <div className="mt-5 grid gap-3 md:grid-cols-3">{data.schedule.map((item) => <Link href="/team/schedules" key={item.id} className="soft-card group relative overflow-hidden p-4 transition hover:-translate-y-0.5 hover:border-[#484843]"><span className={`absolute left-0 top-0 h-full w-0.75 ${accents[item.accent].dot}`}/><div className="flex items-center justify-between"><span className="text-[11px] font-semibold text-[#aaa9a2]">{item.time}</span><div className={`grid h-8 w-8 place-items-center rounded-lg ${accents[item.accent].bg} ${accents[item.accent].text}`}><Icon name={item.icon} className="h-4 w-4"/></div></div><h3 className="mt-5 text-sm font-semibold leading-snug text-[#edede8]">{item.title}</h3><p className="mt-1.5 text-[11px] text-[#777772]">{item.meta}</p></Link>)}</div> : <EmptyState icon="calendar" title="No duties scheduled" body="Duties assigned for today will appear here."/>}</section>
